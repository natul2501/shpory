...
function insertInTextarea(text) {
    ...
    // Додаємо емодзі на місце курсора
    textarea.value = before + text + after;
    
    // Встановлюємо курсор після вставленого емодзі
...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    if (articleId) {
        ...
        form.action = "/diary/public/flysquirrel-diary/newArticleDe";
    }
    ...
    form.addEventListener("submit", async function(event) {
        ...
        try {
            if(articleId){
                ...
                response = await fetch("/diary/public/flysquirrel-diary/newArticleDe", {
                    method: "POST",
                    body: new URLSearchParams(formData), // Відправляємо як form-urlencoded
                    headers: { "Content-Type": "application/x-www-form-urlencoded" }
                });
            ...
            window.location.href = "/diary/public/flysquirrel-diary";
        } catch (error) {
