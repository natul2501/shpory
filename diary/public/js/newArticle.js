...
function insertInTextarea(text) {
    ...
    textarea.setSelectionRange(startPos + text.length, startPos + text.length);// Встановлюємо курсор після вставленого емодзі
    textarea.focus();
}
...
const placeImg = async () => {
    try {
        ...
        let galerieCheckbox = document.getElementById('placeImgInputPrivat');
        let galerieFlag = galerieCheckbox ? galerieCheckbox.checked : false;
        let text = "";
        if(galerieFlag){
            text =
            "<div align=\"center\">\n"+
                "<a href=\"" + link + "\">\n"+
                    "<img src=\"" + link + "\" height=\"400\" title=\"" + description + "\" style=\"border: solid 1px gray\">\n"+
...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    if (articleId) {
        try {
            const response = await fetch(`/diary/public/flysquirrel-diary/getArticle/${articleId}`);
        ...
        form.action = "/diary/public/flysquirrel-diary/newArticle";
    }
    ...
    form.addEventListener("submit", async function(event) {
        ...
        if(content.length < 10){
            document.getElementById("newArticleMessage").textContent = `Необхідно додати текст, не менше 10 символів`;
            return;
        }
        if(thema.length < 10){
            document.getElementById("newArticleMessage").textContent = `Необхідно додати тему, не менше 10 символів`;
            return;
        }
        ...
        for(let part of parts){
            if(part.length>30){
                document.getElementById("newArticleMessage").textContent = `Один тег має містити не більше 30 символів`;
                return;
            }
            if(!validPattern.test(part)){
                document.getElementById("newArticleMessage").textContent = `Некорректний тег ${part}: допустимі літери, цифри та знаки _ - + $ € % & ' "`;
                return;
            }
        }
        ...
        try {
            if(articleId){
                response = await fetch(`/diary/public/flysquirrel-diary/editArticle/${articleId}`, {
                    method: "POST",
                    body: new URLSearchParams(formData), // Відправляємо як form-urlencoded
                    headers: { "Content-Type": "application/x-www-form-urlencoded" }
                });
            } else {
                response = await fetch("/diary/public/flysquirrel-diary/newArticle", {
                    method: "POST",
                    body: new URLSearchParams(formData), // Відправляємо як form-urlencoded
                    headers: { "Content-Type": "application/x-www-form-urlencoded" }
                });
            ...
            return window.location.href = "/diary/public/flysquirrel-diary";
        } catch (error) {
            console.error("Помилка відправки форми:", error);
            document.getElementById("newArticleMessage").textContent = "Помилка збереження статті!";
        }
    });
});
